...
dnl# this in their configure.ac:
...
dnl# configure.ac must invoke:
dnl#   LF_CONFIG_SCRIPT
dnl#
dnl# The package that uses the lib must query the config script installed
...
dnl# Every "library package" corresponds to a config script for that pkg
...
dnl# Now call:
dnl#   LF_QUERY_LIBRARY([lib1],[FOO_CONFIG],[LIB1_LDFLAGS],[LIB1_LDADD],
dnl#                    [LIB1_CFLAGS])
dnl# and you are in bussiness. Instead of lib1 you can request a combo
dnl# of libraries or other libraries.
dnl# (this all just seems like it was a precursor to pkg-config in its day)

dnl#----------------------------------------------------------------------
dnl# Usage:
dnl#   LF_INSTALLS_LIBRARY([libname],[libdir],[libs],[includedir],
dnl#                       [description])
...
dnl# Negotiate whether we have a recent enough ver. of a desired collection
dnl# of libraries.
dnl#-----------------------------------------------------------------------

AC_DEFUN([LF_NEED_LIBRARY_PACKAGE],[
  AC_REQUIRE([AC_PROG_AWK])
  AC_REQUIRE([LF_ENVVARS])
  ...
  then
    ...
    if test "x${LF_DEBUG_MESSAGES}" = "xyes"
    then
...
main()
{
  ...
  if ((major_installed > major_required) ||
    ...
    return 1;
        ...
        echo "** check if you have an old copy also lying around whose"
        echo "** config script is detected before the config script for"
        echo "** the newer version."
        exit 1],[echo ${ac_n} "cross compiling; assumed OK... ${ac_c}"])
  fi
])
 
dnl#-----------------------------------------------------------------------
dnl# Usage:
dnl#  LF_QUERY_LIBRARY_SPECIFIC([libname],[config-script],[flag],
dnl#                            [variable])
...
dnl#-----------------------------------------------------------------------
dnl# Usage:
dnl#   LF_QUERY_LIBRARY([libname],[config-script],[Lflags],[lflags],
dnl#                    [cflags])

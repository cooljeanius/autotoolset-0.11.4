...
AC_DEFUN([LF_LINK_HEADERS],[
  ...
  # Remove the include directory if we have NOT done that yet:
  ...
  if test -z "${ac_aux_dir}"
  then
    export ac_aux_dir=.
  fi
  if test -e ${ac_aux_dir}/mkinstalldirs
  then
    export MKINSTALLDIRS="${ac_aux_dir}/mkinstalldirs"
  elif test -e mkinstalldirs
  then
    export MKINSTALLDIRS=mkinstalldirs
  elif test -e ${srcdir}/mkinstalldirs
  then
    export MKINSTALLDIRS="${srcdir}/mkinstalldirs"
  else
    # could check a few other locations, but why bother?
...
# To use it in your configure.ac call
...
AC_DEFUN([LF_LINK_HEADERS_FUCK],[
  ...
  # If we have not initialized yet, then initialize now:
  ...
  echo "running ${MKINSTALLDIRS} ${lf_directory} 2> /dev/null > /dev/null"
